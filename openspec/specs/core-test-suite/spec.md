...
The automated test suite SHALL verify search-query normalization, track parsing,
session lifecycle, provider-error translation, successful downloads, and the
configured maximum download-size guard.

#### Scenario: Provider markup is valid

- **WHEN** representative playlist markup contains artist, title, and audio URL
  data
...
- **WHEN** a request fails, required playlist data is absent, or a declared
...
- **THEN** the suite verifies the resulting provider URL is normalized and safe
  for use as a host name
